...
function helper_initboard_click(pos) {
  var input=document.getElementById("input_initboard_pos"+pos);
...
function parse_setup() {
  ...
  game_board=[];
  for(let i=0;i<9;++i) {
    var input=document.getElementById("input_initboard_pos"+i);
    switch(input.innerHTML) {
      case "":
        game_board[i]=-1;
        break;
  ...
  //Player setup
  var input_order=document.getElementById("input_order_X");
...
function game_nextturn() {
  ...
  var ui_startgame=document.getElementById("ui_startgame");
  var ui_humanturn=document.getElementById("ui_humanturn");
  var ui_cputurn=document.getElementById("ui_cputurn");
  
  if(is_terminal(game_board)) {
    //Game finished
    helper_log_write("Game complete.");
    var u=utility(game_board,human_player);
    ...
    for(let i=0;i<9;++i) { //Configure input
      var input_playermove=document.getElementById("input_playermove_pos"+i);
      var disp_board=document.getElementById("disp_board_pos"+i);
...
function helper_log_board(b) {
  var bd=document.getElementById("ui_boarddisplay");
  bd=bd.firstElementChild; //Grab object inside
  bd=bd.cloneNode(true);
  
  for(let i=0;i<9;++i) {
    var d=bd.getElementById("boarddisplay_pos"+i);
    switch(b[i]) {
      case -1:
        d.innerHTML="";
        break;
