//Alison (Yutong) Liu, Christopher Zeng
...
function calc_linLSQ_line(data) {
  ...
  ***********************/
  //let p1 = numeric.dot(numeric.inv(numeric.dot(numeric.transpose(A),A)),numeric.transpose(A));
  let p=numeric.dot(numeric.dot(numeric.inv(numeric.dot(numeric.transpose(A),A)),numeric.transpose(A)),b) ;
  //let p = numeric.dot(p1,b);
  ...
  for(let i=0;i<N;++i) {
    ...
    ***********************/
   sse = sse + Math.pow((model_out - y[i]),2);
...
function calc_jacobian(data,p) {
  ...
  for(let i=0;i<N;++i) {
    ...
    */
    ...
    J[i][2]=Math.log(x[i]) * Math.pow(x[i], p[2])*p[3];
    J[i][1]=x[i];
    J[i][0]=1;
...
function calc_nonlinLSQ_gaussnewton(data,initial_p,max_iterations) {
  ...
  for(let iter=0;iter<=max_iterations;++iter) {
    ...
    */
    ...
    // let calc1 = numeric.dot(Jt, J);
    // let calc2 = numeric.inv(calc1);
    // let calc3 = numeric.dot(calc2, Jt);
    // let dp= numeric.dot(calc3, dy);
    let dp=numeric.dot(numeric.dot(numeric.inv(numeric.dot(numeric.transpose(J),J)),numeric.transpose(J)),dy) ;
...
function calc_nonlinLSQ_gradientdescent(data,initial_p,max_iterations,learning_rate) {
  ...
  for(let iter=0;iter<=max_iterations;++iter) {
    ...
    */
      let J = calc_jacobian(data,p);
      let Jt = numeric.transpose(J);
    let grad= numeric.dot(numeric.mul(-2, Jt), dy);
